...
var _initSwipers = require("./modules/swiper/init-swipers");
// ---------------------------------
...
},{"./modules/form-validate/form":3,"./modules/swiper/init-swipers":9,"./modules/tabs/init-tabs":10,"./modules/video/video":12,"./utils/ios-vh-fix":15}],2:[function(require,module,exports){
"use strict";
...
var _swiperBundle = _interopRequireDefault(require("../../vendor/swiper-bundle"));
var _debounce = _interopRequireDefault(require("../../utils/debounce"));
function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }
var swipers = document.querySelectorAll('.swiper'); // коллекция нодов свайперов
var coachSlides = document.querySelector('.swiper--coach').querySelectorAll('.swiper__slide'); // слайды с тренерами
var feedbackSlides = document.querySelector('.swiper--feedback').querySelectorAll('.swiper__slide'); // слайды с тренерами
var coachScrollables = getScrollables(coachSlides, 12);
var feedbackScrollables = getScrollables(feedbackSlides, 21);
resetTabindex(coachScrollables);
resetTabindex(feedbackScrollables);
var DEBOUCE_DELAY = 500;
var TRANSITION_DURATION = 400;
var CoachSlidesPerView = {
  MOBILE: 1,
  TABLET: 2,
  DESKTOP: 4
};
...
var COACHSETTINGS = {
  ...
  // предотвращает переключение сладов при табах
  breakpoints: {
    320: {
      spaceBetween: 20
    },
...
function getScrollables(collection, diff) {
  var output = [];
  Array.from(collection).forEach(function (item, i) {
    output[i] = null;
    var inner = Array.from(item.querySelectorAll('div, p'));
...
function setTabIndex(slides, scrolls) {
  var slidesArray = Array.from(slides); // массив сладйов
  slidesArray.forEach(function (slide, index) {
    var card = slide.querySelector('div'); // установка табиндекса для карточек в видимых слайдах
...
var initSwipers = function initSwipers() {
  var set = getParameters(); // массив с параметрами
  var swipersArray = [];
  if (swipers.length) {
    // создание свайперов, при наличии подходящих нодов
    swipers.forEach(function (item, index) {
      swipersArray[index] = new _swiperBundle.default(".".concat(item.classList[2]), set[index]);
    });
    var coachSwiper = swipersArray[0];
    var feedbackSwiper = swipersArray[1];
    ...
    coachSwiper.on('slideChange', function () {
      return setTimeout(function () {
        return setTabIndex(coachSlides, coachScrollables);
      }, TRANSITION_DURATION);
    });
    feedbackSwiper.on('slideChange', function () {
      return setTimeout(function () {
        return setTabIndex(feedbackSlides, feedbackScrollables);
      }, TRANSITION_DURATION);
    });
  }
};
...
},{"../../utils/debounce":13,"../../vendor/swiper-bundle":16}],10:[function(require,module,exports){
"use strict";
...
var videoSRC = document.querySelector('#player').dataset.videoSrc;
function initVideo() {
  function loadAPI() {
    return new Promise(function (resolve, reject) {
      ...
      tag.onerror = function () {
        return reject(new Error('Ошибка загрузки YouTube API'));
      };
      firstScriptTag.parentNode.insertBefore(tag, firstScriptTag);
    });
  }
  var promise = loadAPI();
  promise.then(onYouTubeIframeAPIReady).catch(function (err) {
    return console.error("\u041E\u0448\u0438\u0431\u043A\u0430: ".concat(err.message));
...
(function (global, factory) {
  ...
  function updateSlidesProgress(translate) {
    ...
    for (var i = 0; i < slides.length; i += 1) {
      ...
      var slideProgress = (offsetCenter + (params.centeredSlides ? swiper.minTranslate() : 0) - slideOffset) / (_slide3.swiperSlideSize + params.spaceBetween);
      var originalSlideProgress = (offsetCenter - snapGrid[0] + (params.centeredSlides ? swiper.minTranslate() : 0) - slideOffset) / (_slide3.swiperSlideSize + params.spaceBetween);
      ...
      if (value) {
        for (var i = 0, length = swiper.slides.length; i < length; i += 1) {
          ...
          if (slideHistory === value) {
            var index = elementIndex(_slide4);
  ...
  function HashNavigation(_ref) {
    ...
    var onHashChange = function onHashChange() {
      ...
      var activeSlideHash = swiper.slides[swiper.activeIndex].getAttribute('data-hash');
      if (newHash !== activeSlideHash) {
        var newIndex = elementIndex(elementChildren(swiper.slidesEl, ".".concat(swiper.params.slideClass, "[data-hash=\"").concat(newHash, "\"], swiper-slide[data-hash=\"").concat(newHash, "\"]"))[0]);
    ...
    var setHash = function setHash() {
      ...
      if (swiper.params.hashNavigation.replaceState && window.history && window.history.replaceState) {
        ...
        var _slide5 = swiper.slides[swiper.activeIndex];
        var hash = _slide5.getAttribute('data-hash') || _slide5.getAttribute('data-history');
    ...
    var init = function init() {
      ...
      if (hash) {
        var speed = 0;
        for (var i = 0, length = swiper.slides.length; i < length; i += 1) {
          var _slide6 = swiper.slides[i];
          var slideHash = _slide6.getAttribute('data-hash') || _slide6.getAttribute('data-history');
          if (slideHash === hash) {
            var index = elementIndex(_slide6);
  ...
  function effectVirtualTransitionEnd(_ref) {
    ...
    var getSlide = function getSlide(el) {
      if (!el.parentElement) {
        // assume shadow root
        var _slide7 = swiper.slides.filter(function (slideEl) {
